...
<Tabs>
  ...
  <TabItem label="SQLite">
    ...
    const client = new HttpClient({
      name: 'my-api',
      cache: { store: new SQLiteCacheStore({ database: db }) },
      dedupe: new SQLiteDedupeStore({ database: db }),
      rateLimit: { store: new SQLiteRateLimitStore({ database: db }) },
    });
    ```
  </TabItem>
  <TabItem label="DynamoDB">
    ...
    const client = new HttpClient({
      name: 'my-api',
      cache: { store: new DynamoDBCacheStore({ client: dynamoClient }) },
      dedupe: new DynamoDBDedupeStore({ client: dynamoClient }),
      rateLimit: { store: new DynamoDBRateLimitStore({ client: dynamoClient }) },
    });
    ```
  </TabItem>
</Tabs>
...
const client = new HttpClient({
  name: 'my-api',
  cache: { store: new InMemoryCacheStore() },               // Fast local cache
  rateLimit: { store: new DynamoDBRateLimitStore({ ... }) }, // Shared rate limit
});
```
