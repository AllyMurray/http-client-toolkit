...
The `createDynamoDBStores` factory is the recommended way to set up all stores with a shared client:
...
You can pass an existing client and per-store options:
...
const stores = createDynamoDBStores({
  client: dynamoClient,
  rateLimit: { defaultConfig: { limit: 100, windowMs: 60_000 } },
});
```

When you pass your own client, the factory will **not** destroy it on `close()` — you retain ownership.

## Individual Stores

You can also create stores individually. All accept a `DynamoDBDocumentClient`, a plain `DynamoDBClient` (auto-wrapped), or no client (created internally with optional `region`):
