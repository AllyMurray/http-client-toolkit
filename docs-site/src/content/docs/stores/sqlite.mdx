...
The `createSQLiteStores` factory is the recommended way to set up all stores with a shared database connection:
...
// Cleanup: closes all stores and the shared database connection
await stores.close();
```

The factory accepts per-store options:
...
By default, the factory uses `':memory:'` (non-persistent).

## Individual Stores

You can also create stores individually. Each accepts either a **file path** or an existing **better-sqlite3 Database instance**.
