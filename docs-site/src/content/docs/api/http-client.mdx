...
| `cache` | `{ ttl?: number; overrides?: CacheOverrideOptions }` | — | Per-request cache options. `ttl` overrides constructor TTL; `overrides` are shallow-merged with constructor-level |
