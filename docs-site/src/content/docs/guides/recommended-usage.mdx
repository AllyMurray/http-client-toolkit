...
```typescript
// lib/stripe.ts — SQLite for persistence
import { HttpClient } from '@http-client-toolkit/core';
import { createSQLiteStores } from '@http-client-toolkit/store-sqlite';

const stores = createSQLiteStores({
  database: './stripe-store.db',
  rateLimit: { defaultConfig: { limit: 25, windowMs: 1_000 } },
});
...
When multiple clients should share the same underlying storage — for example, a single SQLite database or a shared cache pool — pass the same store instances to each client.

For persistent backends like SQLite, use the factory to create a shared set of stores with a single database connection:
...
The same pattern works with in-memory stores when you don't need persistence:
...
Cache and dedup are shared so both clients write to and read from the same pool. Each client's cache keys are automatically prefixed with its name (e.g. `github:` and `stripe:`), so entries don't collide even in the same store. Rate limiting is kept separate because each API enforces its own limits.

This also means the [dashboard](/dashboard/overview/) shows the same cache data regardless of which client you select.
