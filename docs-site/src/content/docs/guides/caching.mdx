...
4. No freshness info — falls back to `cacheTTL`
...
Both `cacheTTL` and `cacheOverrides` can be set per request, overriding the constructor defaults. This is useful when different endpoints have different caching needs.
...
Per-request `cacheOverrides` are **shallow-merged** with the constructor-level overrides — specified fields override, unspecified fields fall back to the constructor defaults:

```typescript
const client = new HttpClient({
  cache: new InMemoryCacheStore(),
  cacheTTL: 300,
  cacheOverrides: { minimumTTL: 60, maximumTTL: 3600 },
});

// minimumTTL overridden to 120, maximumTTL stays 3600
await client.get(url, {
  cacheOverrides: { minimumTTL: 120 },
});
```
