...
Mount the dashboard inside an existing HTTP server using `createDashboard()`. Pass in one or more named clients, each with their own store instances:
...
```typescript
const handler = createDashboardHandler({
  clients: [{ name: 'my-api', cacheStore }],
  basePath: '/dashboard',
});
```
:::
...
|--------|------|----------|-------------|
| `name` | `string` | Yes | URL-safe identifier for this client |
| `cacheStore` | `CacheStore` | No | Cache store instance to monitor |
| `dedupeStore` | `DedupeStore` | No | Dedup store instance to monitor |
| `rateLimitStore` | `RateLimitStore` | No | Rate limit store instance to monitor |

At least one store must be provided per client.
